...
func runInstallNewComponents(ctx context.Context, commandContext *task.CommandContext) error {
	if err := commandContext.ClusterManager.ApplyBundles(ctx, commandContext.ManagementSpec, commandContext.ManagementCluster); err != nil {
		commandContext.SetError(err)
		return err
	}

	if err := commandContext.ClusterManager.ApplyReleases(ctx, commandContext.ManagementSpec, commandContext.ManagementCluster); err != nil {
		commandContext.SetError(err)
		return err
	}

	err := commandContext.EksdInstaller.InstallEksdManifest(ctx, commandContext.ManagementSpec, commandContext.ManagementCluster)
	if err != nil {
		commandContext.SetError(err)
		return err
	}
